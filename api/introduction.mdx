...
1. First, list available datasets using the [GET /datasets](/api/endpoint/get-datasets) endpoint:

```bash
curl https://portal.sqd.dev/datasets
```

1. Choose a dataset from the response (e.g., `ethereum-mainnet`, `base-mainnet`).
2. Access dataset-specific endpoints by replacing `{dataset}` in the URL with your chosen dataset name:
...
Request the data you need by [`POSTing to /stream`](/api/endpoint/post-stream) and you shall receive a stream of newline-separated JSON objects that package your data by block. For example
...
2. Ensuring data correctness when streaming unfinalized blocks is somewhat complicated. If you don't want to deal with that complexity and don't mind that the newest available data is up to several hours old, use the alternative [`/finalized-stream endpoint`](/api/endpoint/post-finalized-stream); otherwise read on.
...
- [GET /datasets](/api/endpoint/get-datasets) - get list of all available datasets.
- [GET /metadata](/api/endpoint/get-metadata) - dataset metadata. Includes the number of the lowest available slot.
- [GET /head](/api/endpoint/get-head) - slot number and hash of the highest block of the dataset.
- [GET /finalized-head](/api/endpoint/get-finalized-head) - same, but only for finalized blocks. In the current implementation it will be behind `/head` by some thousands of slots.
- [POST /finalized-stream](/api/endpoint/post-finalized-stream) - same as the main endpoint [POST /stream](/api/endpoint/post-stream), but will only return finalized blocks, up to the height you can get from [GET /finalized-head](/api/endpoint/get-finalized-head)
- [GET /state](/api/endpoint/get-state) - information on how the dataset is distributed within the SQD Network.
